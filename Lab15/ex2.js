...
app.post('/process_register', function(req, res) {
    ...
    // save updated user_data to file (database)
    fs.writeFileSync(user_data_file, JSON.stringify(user_data))
    res.send(`${username}username is registered`)
});
 ...
 app.post('/process_login', function (req, res, next) {
    ...
    if (typeof user_data[username_entered] !='undefined') {
        if (user_data[username_entered]['password'] == password_entered){
            res.cookie('username', username_entered, {"maxAge": 10*1000});
            res.send(`${username_entered} is logged in`);
        } else{
            res.send (`${username_entered} password wrong`);
        }
    } else {
        res.send(`${username_entered} not found`)
    }
 }); 
